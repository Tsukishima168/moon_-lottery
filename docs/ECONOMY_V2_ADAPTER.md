...
- `ALREADY_PROCESSED` 必須帶回與原成功相同的 play、outcome、cost、reward、
  committed balance 與 event；缺欄 replay 直接 fail closed，不重新抽選或
  再次發點。
...
2026-07-16 狀態：上述 hosted migration、Supabase lint、Auth/RLS/PostgREST、
replay、原生 100-request 與 hosted concurrency invariant 已通過；production
仍未套用六項 migration，game config/rollout 仍 off，PR #19 仍等待明確的
migration／merge／deploy 授權。
