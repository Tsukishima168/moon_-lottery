...
- 新抓到的每日限制邊界 bug：若 `moonmoon_gacha_today_result` 壞掉或缺失，`isPlayedToday` 仍會是 `true`，但 `resultPrize/resultFortune` 為空，導致使用者能再抽一次。
- 已修正：當今天已抽但結果資料不可讀時，阻止再次抽取，改顯示前台提示要求重新整理後查看。
- 進一步收斂：若 `today_result` 是壞 JSON，現在會清掉壞快取並用 graceful fallback，不再噴 `console.error`。
- 新一輪觀察：雖然轉蛋外層 hit area 已固定，但標準自動 click 仍偶爾判定元素不穩。
- 已修正：轉蛋入口改成真正固定的 overlay button，視覺動畫與可點擊層完全分離。
- 驗證：`develop-web-game` Playwright client 現在可直接用 selector 點進結果 modal，無額外 console error。
- 再次驗證：`npm run build` 通過；`npx tsc --noEmit` 通過。
