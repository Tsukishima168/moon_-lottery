...
以下內容優先於 2026-04-01 的舊分支快照：

- Current branch: `main`
- Latest checked commit: `3559adc fix(gacha): replace wheel emoji with reward codes`
- Current role: Gacha campaign/game center, daily point draw, lucky wheel, Passport point sync, LINE share support
- Build command: `npm run build`
- Verification doc: `VERIFY.md`
- Current state doc: `CURRENT.md`

Current UI language rule:

- Do not use emoji as reward identifiers, wheel labels, CTA labels, or persistent customer-facing UI markers.
- Use text labels, reward codes, lucide icons, colors, and structured point values instead.
...
3. `package.json`
4. `App.tsx`
5. `src/components/LuckyWheel.tsx`
6. `pointsSystem.ts`
7. `wheelService.ts`
8. `src/lib/supabase.ts`
9. `src/lib/auth.ts`
10. `src/lib/liffShare.ts`

Do not use the old feature-branch note below as current state. It is retained only as historical context.
...
**重要：以下是 2026-04-01 的歷史快照，不是 2026-06-04 現況。**
